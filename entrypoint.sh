...
# Written to /tmp (not /etc/xray) because we now run as the non-root
# "nginx" user, which doesn't have write access to /etc/xray.
cp /etc/xray/config.template.json /tmp/xray/config.json
