...
if [ -n "$BALE_COOKIES_B64" ]; then
  echo "$BALE_COOKIES_B64" | base64 -d > /app/bale-cookies.json

  /usr/local/bin/headless-bale-creator \
    --cookies /app/bale-cookies.json \
    --write-file /app/join-link.txt \
    --resources moderate &
  BALE_PID=$!
  ...
  ( while [ ! -f /app/join-link.txt ]; do sleep 1; done
    tail -f /app/join-link.txt | while read -r line; do
